...
class Instance(object):
    ...
    def __init__(self, tokenizer, single_sentence_pack, args):
        ...
        # if len(self.sentence.strip().split(' ')) != len(self.word_spans):
        #     print(self.sentence)
        #     print(self.word_spans)
        ...
        for i in range(min(len(self.bert_tokens),self.args.max_sequence_len)):
    ...
    def get_mask(self):
        ...
        mask[len(self.bert_tokens):, :] = 0
        # if len(self.bert_tokens)>self.args.max_sequence_len:
        #     print(len(self.bert_tokens), self.args.max_sequence_len)
        for i in range(min(len(self.bert_tokens),self.args.max_sequence_len)):
            mask[i][i] = 0
        return mask
